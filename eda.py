...
SERIES_COLS = [
    "match_id", "tournament_id", "tournament_en", "team1_id", "team1",
    "team2_id", "team2", "score1", "score2", "bestOf", "bestOf_clean",
    "datetime", "year", "month", "team1_win", "games_played",
]
...
def overview(df: pd.DataFrame) -> None:
    ...
    print("\nПример строк:")
    show = ["match_id", "game_id", "tournament_en", "team1", "team2",
            "score1", "score2", "bestOf", "team1_win", "datetime"]
...
def clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"bestOf: {int(bad_bo.sum())} аномальных значений "
...
def missing_values(df: pd.DataFrame) -> None:
    ...
    if len(miss):
        ...
        sns.barplot(x=miss.values, y=miss.index, hue=miss.index,
...
def series_analysis(df: pd.DataFrame) -> pd.DataFrame:
    ...
    sns.barplot(x=cards_per.value_counts().sort_index().index,
    ...
    sns.barplot(x=bo.index.astype(int).astype(str), y=bo.values,
                hue=bo.index, palette="flare", legend=False, ax=axes[1])
...
def target_balance(series: pd.DataFrame) -> None:
    ...
    print("(заметный перекос от 0.5 = порядок team1/team2 несёт информацию,"
    ...
    sns.barplot(x=by_bo.index.astype(int).astype(str), y=by_bo.values,
                hue=by_bo.index, palette="viridis", legend=False, ax=ax)
    ax.axhline(0.5, ls="--", c="red", lw=1)
    ax.set(title="Доля побед team1 по bestOf", xlabel="Best of",
           ylabel="P(team1_win)", ylim=(0, 1))
...
def teams_analysis(series: pd.DataFrame) -> None:
    ...
    agg = long.groupby("team").agg(series_played=("win", "size"),
                                   winrate=("win", "mean"))
    ...
    qualified = agg[agg["series_played"] >= 50].sort_values(
        "winrate", ascending=False)
    print("\nТоп-10 по винрейту (≥50 серий):")
    print(qualified.head(10).round(3).to_string())

    fig, axes = plt.subplots(1, 2, figsize=(15, 6))
    sns.barplot(x=top_games["series_played"], y=top_games.index,
                hue=top_games.index, palette="mako", legend=False, ax=axes[0])
    axes[0].set(title="Топ-15 команд по числу серий", xlabel="серий", ylabel="")
    qt = qualified.head(12)
    sns.barplot(x=qt["winrate"], y=qt.index, hue=qt.index,
...
def tournaments_and_coverage(df: pd.DataFrame, series: pd.DataFrame) -> None:
    ...
    by_year = (df.dropna(subset=["year"])
                 .groupby(df["year"].astype("Int64"))["has_game_data"].mean())

    fig, axes = plt.subplots(1, 2, figsize=(15, 5))
    sns.barplot(x=top_t.values, y=top_t.index, hue=top_t.index,
                palette="crest", legend=False, ax=axes[0])
    axes[0].set(title="Топ-15 турниров по числу серий", xlabel="серий", ylabel="")
    sns.lineplot(x=by_year.index.astype(int), y=by_year.values,
                 marker="o", ax=axes[1])
    axes[1].set(title="Доля карт с детальной статистикой по годам",
                xlabel="год", ylabel="has_game_data", ylim=(0, 1))
